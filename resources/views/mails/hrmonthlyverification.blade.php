                ...
                <table class="one-column" border="0" cellpadding="0" cellspacing="0" width="100%" style="border-spacing:0; border-left:1px solid #e8e7e5; border-right:1px solid #e8e7e5; border-bottom:1px solid #e8e7e5; border-top:1px solid #e8e7e5" bgcolor="#FFFFFF">
                    ...
                    <strong>Dear {{$greetingsName}}</strong>,</p>
                      <p style="color:#000000; font-size:16px; text-align:left; font-family: Verdana, Geneva, sans-serif; line-height:22px ">
                            Please find attached the list of the verified Mds for the month of {{$month}} under zone <strong> {{$zone}} </strong> and ZBM <strong>{{$fullName}}</strong>
                    @foreach ($mds as $item)
                        <p>{{$loop->index+1}}. FullName: <strong>{{$item->userprofile->last_name.''.$item->userprofile->first_name}}</strong> &nbsp; &nbsp; Territory: <strong>{{$item->territory->name}}</strong></p>
                    @endforeach
