...
@section('content')
      <!-- Page Content -->
        <div class="page-wrapper" data-ng-controller="InboxController">
            <div class="container-fluid">
             <!-- row -->
                <div class="row">
                            ...
                            <div class="row">
                                <div class="col-lg-2 col-md-3  col-sm-12 col-xs-12 inbox-panel">
                                    <div> <a href="{{route('inbox.create')}}" class="btn btn-custom btn-block waves-effect waves-light">Compose</a>
                                        <div class="list-group mail-list m-t-20"> <a href="{{route('inbox.index')}}" class="list-group-item {{(($active=='inbox') ? 'active' : '')}}">
                                        Inbox <span class="label label-rouded label-success pull-right">
                                        {{$inboxCount}}</span></a> <a href="{{route('inbox.category','starred')}}" class="list-group-item {{(($active=='starred' ? 'active' : ''))}}">
                                        Starred <span class="label label-rouded label-default pull-right">
                                        {{$starredCount}}</span></a> 
                                        <a href="{{route('inbox.category','draft')}}" class="list-group-item {{(($active=='draft') ? 'active':'')}}">Draft
                                         <span class="label label-rouded label-warning pull-right">
                                         {{$draftedCount}}</span></a> <a href="{{route('inbox.sent')}}" class="list-group-item">Sent Mail
                                        <span class="label label-rouded label-primary pull-right">{{$sentCount}}</span>
                                        </a> <a href="{{route('inbox.category','trash')}}" class="list-group-item {{(($active=='trash') ? 'active' : '')}}">Trash 
                    ...
                    <td class="hidden-xs "><i id="starred{{$item->id}}" class="fa {{(($item->is_starred) ? 'fa-star text-warning' : 'fa-star-o ')}}"></i></td>
                                                        <td class="hidden-xs">
                                                        @if(isset($item->senderProfile))
                                    ...
                                    <div class="row">
                                        <div class="col-xs-7 m-t-20"> {{$inbox->links()}} </div>
                                        <div class="col-xs-5 m-t-20">
                                            <div class="btn-group pull-right">
                                                Total Results : {{$contentCount}}
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@section('pagejs')
    <script src="{{asset('js/controllers/inbox.js')}}"></script>
@endsection