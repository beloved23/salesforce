...
<body data-ng-app="myApp" data-ng-controller="myController">
    ...
    <section id="wrapper" class="login-register" style="background: url({{asset('images/slava-bowman.jpg')}}) center/cover no-repeat !important;">
        <div class="login-box" style="margin: 4% auto 0 !important">
            <div class="white-box">
           <div class="text-center"> <h2>Select your account</h2></div>
                      <div class="message-center">
                                 @foreach($allHR[0]->users as $item)
                                       <a data-ng-click="onSelectAccount({{$loop->index}})" data-toggle="modal" data-target="#responsive-modal" href="javascript:void(0);">
                                        <div class="user-img">
                                            <img src="{{asset('storage')}}/{{$item->profile->profile_picture}}" alt="user" class="img-circle"><span class="profile-status away pull-right"></span>
                                        </div>
                                        <div class="mail-contnet">
                                            <h5>{{$item->profile->first_name.' '.$item->profile->last_name}}</h5>
                                            <span class="mail-desc">{{$item->email}}</span>
                                            <span class="time">Auuid: {{$item->id}}</span>
                                        </div>
                                    </a>
                                 @endforeach
                                </div>
                                        ...
                                        <div class="modal-body">
                                            <form>
                                                <div class="form-group">
                                                    <label for="recipient-name" class="control-label">HR: @{{hrFullName}} <span class="m-l-10">Email: @{{hrEmail}}</span></label>
                                                <div class="form-group ">
                                                    <label for="message-text" class="control-label">Password</label>
                                                    <input data-ng-model="password" data-ng-keyup="authenticateOnEnter($event)" type="password" class="form-control" placeholder="Password" id="message-text"></input>
                                                </div>
...
<script src="{{ asset('js/toastr.min.js') }} "></script>
    ...
    app.controller('myController',function($scope,$http){
        $scope.allHr = allHR;
        $scope.roleMovementId = roleMovementId;
 ...
 ShowGlobalLoader();
                    var data = "hrAuuid=" + encodeURIComponent($scope.selectedHr.id) + "&token=" + encodeURIComponent($scope.token) 
                     + "&roleMovementId="+encodeURIComponent($scope.roleMovementId)+"&password="+encodeURIComponent($scope.password);
                    var config = { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } };
                    $http.post(app_url + "role/movement/claim/verify", data, config).success(function(data) {
                        HideGlobalLoader();
                          if (data.validations && data.action) {
                            GlobalSuccessNotification(data.message);
                            setTimeout(function(){
                            window.location.href = app_url+'login?token='+data.userToken+"&redirect_route="+data.routeToken+"&id="+$scope.roleMovementId;
                            },3000);
                        } else {
