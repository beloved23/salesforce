...
@section('content')
 <!-- Page Content -->
        <div class="page-wrapper" data-ng-controller="ShowLgaController">
            <div class="container-fluid">
<div class="row">     <div id="showZoneTable" class="col-lg-12">
                        <div class="white-box">
                            <h3 class="box-title">Geography: Lga</h3>
                            <!--Zone table -->
                            <table class="table table-striped table-bordered" id="example23">
                                ...
                                @foreach($lgas as $lga)
                                    <tr class="gradeC">
                                        ...
                                        <td>{{$lga->lga_code}}</td>
                                        <td>{{$lga->area_id}}</td>
                                        <td class="center">{{$lga->area->name}}</td>
                                        @if(isset($lga->area->asmByLocation))
                            <td class="center">{{$lga->area->asmByLocation->userprofile->first_name.' '.$lga->area->asmByLocation->userprofile->last_name}}</td>
                                        @else
 ...
 <!--Details Section -->
    ...
    @endsection
            @section('pagejs')
    <script src="{{asset('js/controllers/show.lga.js')}}"></script>
