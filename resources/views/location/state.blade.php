...
@section('content')
 <!-- Page Content -->
        <div class="page-wrapper" data-ng-controller="ShowStateController">
            <div class="container-fluid">
<div class="row">     <div id="showZoneTable" class="col-lg-12">
                        <div class="white-box">
                            <h3 class="box-title">Geography: State</h3>
                            <!--Zone table -->
                            <table class="table table-striped table-bordered" id="example23">
                                ...
                                @foreach($states as $state)
                                    <tr class="gradeC">
                                        ...
                                        <td>{{$state->zone_id}}</td>
                                        <td class="center">{{$state->zone->name}}</td>
                                        <td class="center">{{$state->zone->zone_code}}</td>
                                     <td class="center">
                                     @isset($state->zone->zbmByLocation)
                    {{$state->zone->zbmByLocation->userprofile->first_name}}
                                     @endisset
                                     N/A
                                     </td>
 ...
 <!--Details Section -->
    ...
    @endsection
            @section('pagejs')
    <script src="{{asset('js/controllers/show.state.js')}}"></script>
